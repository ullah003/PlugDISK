...
public class F004 extends BaseTestLogic {
	
	private static String System_Out = "", parent_shareName = "", sub_shareName = "", subShare_diskName = "";
	int i, j, k, totl_user = 0;
	private static String [] user_list, Success;
	Boolean parentPresent = false, sub_sharePresent = false;
	
	public F004(WebDriver webDriver, File fileConf, File fileReport) {
	...
	void startTest() throws InterruptedException {
		System.out.println("*****************************************************************");
		System.out.println("*                    Sub-Share Permission Setting               *");
		System.out.println("*****************************************************************");
		System.out.println("Test F004 - Sub_Share - Permission Setting");
		System.out.println("Sub_Share permission setting can be done for top share? \n" +
							"서브 공유 디스크 접근 권한 설정? \n"+
							"-----------------------------------------------------------------");
	        ...
	        for (int i=1; i<=43; i++){
	        	...
	        	if (i > 41){
		        	...
		        	if (tokens[0].equals("parent_share")){
		        		parent_shareName = tokens[1];
		        	}else if (tokens[0].equals("subshare_name")){
		        		sub_shareName = tokens[1];
		        	}
	        	}
	        }
	    } finally {
		...
		for (WebElement parent_share : all_parentshare) {
			//System.out.println(parent_share.getText());
			if (parent_share.getText().equals(parent_shareName)){
				parentPresent = true;
				parent_share.click();
				break;
			}
  		...
  		if (parentPresent){
			...
			for (WebElement sub_share : all_subshare) {
				...
				i = i + 1;
				if (sub_share.getText().equals(sub_shareName)){
					driver.findElement(By.xpath("//div[@id='share-list']/table/tbody/tr[" + i + "]/td/input")).click();
					subShare_diskName = driver.findElement(By.xpath("//div[@id='share-list']/table/tbody/tr[" + i + "]/td[6]")).getText();
					System.out.println("=============================>" + subShare_diskName);
			...
			if (sub_sharePresent){
				...
				System.out.println("Total number of users permited to access sub_share is: " + j);
				driver.findElement(By.id("okButton")).click();		//check OK 
				Thread.sleep(2000);
				driver.switchTo().defaultContent();
				
				//go to Share menu and get list of users
				...
				for (i=0; i<totl_user; i++) {
					...
					Success[i] = "User (" + user_list[i] + ") is not granted permission to sub_share " + sub_shareName + " (FAIL).";
					for (WebElement user_share : user_allshare) {
						//System.out.println(user_list[i] + "-----------" + user_share.getText());
						
						//if (user_share.getText().equals(sub_shareName)){
						if (user_share.getText().equals(subShare_diskName)){
							Success[i] = "User (" + user_list[i] + ") has been granted permission to sub_share " + sub_shareName + " successfully.";
							break;
						}
				...
				System_Out = "The sub_share you want to set permission is not available.";
  		}else
  			System_Out = "Parent share not present to set permission to sub_share.";
		...
		try {
	        ...
	        builder.append("\r\n*****************************************************************\r\n");
	        builder.append("*                    Sub-Share Permission Setting               *\r\n");
	        builder.append("*****************************************************************\r\n");
	        builder.append("Test F004 - Sub_Share - Permission Setting\r\n");
	        builder.append("Sub_Share permission setting can be done for top share? \r\n" +
						   "서브 공유 디스크 접근 권한 설정?\r\n");
	        builder.append("=================================================================\r\n");
	        builder.append(System_Out + "\r\n");
	        builder.append("Total number of users permited to access sub_share is: " + j + "\r\n");
